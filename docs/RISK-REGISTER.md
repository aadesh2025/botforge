...
| R14 | Reliability / data integrity | **⚠️ NEW, found by R4's own load test, not fixed.** A visitor whose client disconnects mid-stream (tab closed, navigated away, or simply the common pattern of reading only the first SSE event) — **100% reproducible, not a race** — never gets their assistant reply persisted to conversation history, and the server logs an `unhandled_exception` (a `CancelledError` through `BaseHTTPMiddleware`, landing on a SQLAlchemy session mid-flush) with no signal to the client (HTTP is already `200`, headers already on the wire). | High (silent data loss in a chat product's own history) | Certain today — reproduced 38/38 times under the R4 load test, and a matched control (full drain) persisted correctly every time | docs/15 §11.2: 53/53 user messages from the concurrent chat load test have zero matching assistant message. | Needs a design decision before a fix: does a disconnected stream persist whatever was generated so far, or discard on purpose; does the middleware order/choice change; does persistence move earlier in `app/chat/runtime.py` so a cancellation can't land before it. Touches `app/main.py`'s middleware stack and/or the chat streaming/persistence path — flag to whoever owns that design, don't guess at it. |
...
- **Before onboarding client #1 with real data:** clear R2, R3, and **R14** at minimum (**R1
  closed 2026-09-23**, ADR-082; **R4 measured 2026-09-23**, see docs/15 §11). R2 is a same-day
  fix (it's a document, not code). **R14 is new and arguably more urgent than R4 ever was** —
  it's a concrete, 100%-reproducible data-loss bug, not an absence of measurement. R3 is the one
