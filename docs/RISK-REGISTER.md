...
|----|------|------|----------|------------|-----|---------------------|
| R2 | PII / pre-launch cleanup | **Not a live exposure — a pre-launch checklist item.** Originally logged (2026-08-03) as "a live client KB document (aurozenai) is flagged, not cleaned"; that overstated it. There is **no production deployment** (docs/16: "NOT YET EXECUTED"), so nothing is served to real traffic; this was pre-launch content in a local dev environment. Remaining task: make sure nothing unredacted is carried into the Supabase database (ADR-086). Egress redaction stays a backstop, not a fix. | Medium | Low (no real traffic; becomes real only at first production ingestion) | **Checked 2026-09-24:** `audit_kb_pii.py` on the reachable dev DB → 1 ingested document across 55 orgs, clean; the `aurozenai` org is not in it. `apps/api/var/uploads/` held **four** (not three) flagged files with no DB row pointing at them: the 2026-07-20 PDF (AUROZEN AI's own KB — the flagged email/phone are the operator's own business contact) and three 2026-08-06 `.docx` sample KBs that were fictional (`*.example` domains, 555 numbers). The three fictional files were deleted 2026-09-24; the PDF was left untouched. Dev leftovers, not served to anyone. | **Before the first production ingestion:** decide for the PDF's contact details — allow-list them via `organizations.public_contacts` (the designed mechanism; an agent that can't give out its own support address is broken) or redact the source before upload. **Then verify with `audit_kb_pii.py` against the Supabase DB once it is live — not before.** No incident response needed. |
...
- **Before onboarding client #1 with real data:** clear R3 at minimum (and run the R2 pre-launch check) (**R1
  closed 2026-09-23**, ADR-082; **R4 measured 2026-09-23**, see docs/15 §11; **R14 fixed
  2026-09-24**, ADR-083). R2 is a same-day
  check against the new database (a document, not code — and not an incident). R3 is the one
