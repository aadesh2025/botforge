...
| R4 | Reliability | **No load or scale testing has ever been performed.** Every verification in the session log is correctness under low, sequential, test-shaped traffic. | High | Rises fast with real usage | Nothing in `docs/PROGRESS.md` or `CLAUDE.md` §11 mentions throughput/concurrency testing at any point. | Basic load test (concurrent chat sessions, concurrent workflow runs) before promising any client an SLA. |
...
- **Before onboarding client #1 with real data:** clear R2-R4 at minimum (**R1 closed
  2026-09-23**, ADR-082). R2 is a same-day fix (it's a document, not code). R4 is a small,
  bounded engineering task. R3 is the one that needs a human decision (budget/vendor for an
  external review) — flag it to whoever owns that budget rather than trying to self-serve it
  with more self-authored red-team fixtures, which is the exact blind spot R3 describes.
- **Before promising any uptime/response-time SLA:** R4 (load testing) is not optional.
